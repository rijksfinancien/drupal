...
namespace Drupal\minfin_piwik\Form;
...
class PiwikDataLayerForm extends FormBase {
  ...
  public function getFormId() {
    return 'minfin_piwik_settings_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['row']['help'] = [
      ...
      '#type' => 'link',
      '#url' => Url::fromRoute('minfin_piwik.list_all_routes'),
      '#attributes' => [
        'class' => ['use-ajax'],
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $this->connection->truncate('minfin_piwik_datalayer')->execute();

    foreach ($form_state->cleanValues()->getValue('dataLayer', []) as $value) {
      if (!empty($value['route']) && !empty($value['page_type'])) {
        $this->connection->insert('minfin_piwik_datalayer')
          ->fields([
            'route' => $value['route'],
